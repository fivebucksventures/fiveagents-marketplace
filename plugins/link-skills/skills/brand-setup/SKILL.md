...
**v2.7.1** — May 20, 2026
- Step 8d readiness now reads the generated `skills-manifest.json` (each skill's `area` + `deps` from its `SKILL.md` frontmatter) instead of parsing the `agents/link.md` Skills table — the table is now generated by `scripts/gen_skills_index.py`. 8d-iii groups by `area` from the manifest; 8d-iv derives `connected_tools[]` from `deps.mcp`/`deps.gateway`. No change to the readiness output; removes the table-parse coupling (Phase 0 of scaling to 300+ skills).
- Step 8d-i translation table: replaced the dead `gateway: templates` row with `gateway: fivebucks → "Branded social templates"` (left over from the v2.7.0 fb.ai migration — skills now declare `fivebucks` as the gateway dep, so the readiness matrix would have printed the raw token instead of a business label).
