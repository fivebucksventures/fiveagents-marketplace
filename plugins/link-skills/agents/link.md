...
| `data-analysis` | Marketing | Analyze campaign performance data — KPI dashboards, weekly/monthly reports, traffic and lead analysis | MCP: Windsor.ai · Gateway: Zernio (ads) (opt — fallback data pull + act-on-findings) · Files: brand.md, funnel.md · Env: `${BRAND}_LATE_GOOGLE_ADS` + `${BRAND}_LATE_GOOGLE_ADS_CID` (opt — Google Ads Zernio fallback pair), `${BRAND}_LATE_META_ADS_ACCOUNT_ID` (opt — Meta Ads Zernio fallback), `${BRAND}_LATE_LINKEDIN_ADS` + `${BRAND}_LATE_LINKEDIN_ADS_CID` (opt — LinkedIn Ads Zernio fallback pair) |
...
| `digital-marketing-analyst` | Marketing | Daily and weekly paid ads analysis — Google Ads, Meta Ads, LinkedIn Ads (opt), GA4 funnel analysis with structured JSON email briefs | MCP: Windsor.ai, Slack, Gmail (opt — fallback when fiveagents_send_email returns 403), Meta Ads MCP (opt — Windsor covers Meta when absent) · Gateway: email, Zernio (ads) (opt — Windsor fallback data pull; Phase 4 ads actions) · Files: brand.md, funnel.md · Env: `${BRAND}_LATE_GOOGLE_ADS` + `${BRAND}_LATE_GOOGLE_ADS_CID` (opt — Google Ads Zernio fallback pair), `${BRAND}_LATE_META_ADS_ACCOUNT_ID` (opt — Meta Ads Zernio fallback), `${BRAND}_LATE_LINKEDIN_ADS` + `${BRAND}_LATE_LINKEDIN_ADS_CID` (opt — LinkedIn Ads Zernio fallback pair; gates Phase 2.5 and weekly Step 1d) |
...
| `investor-update-writer` | Strategy | Monthly investor update — branded Gamma deck (Google Doc fallback) plus per-investor Gmail draft. Combines Xero financials + PostHog product KPIs + CRM wins, drafts in founder voice | MCP: Xero, Stripe, PostHog, Notion, Gmail, Slack, Gamma, Google Drive (fallback when Gamma fails) · Files: investors.md, finance.md, brand.md, product.md, design-system/ (opt — informs Gamma deck visual identity when present, brand.md fallback otherwise) · Env: `${BRAND}_CRM_DB`, `${BRAND}_REPORTS_DB` |
